...
* The official plugin was opinionated in ways I don't like, mostly irrelevant Asciidoc document-based view.
Like h1 doesn't create a proper slide.
====
...
* Are
* Here emoji:muscle[]
* Basically just include the Notes plugin by default.
It's the only one I've kept, the rest doesn't need to be interactive.
====
...
[.notes]
====
* GitHub-like integration of Mermaid in the deck allows for integrating the graph as an SVG inside the presentation.
SVG is *light*!
Even more so after gzip!
* Displaying bit-by-bit is easy-ish.
Could be better, but a simple format allows to fragment the graph
* Switching the global theme switches the highlighting theme too!
====

=== Mermaid graphs — animations
...
----
architecture-beta
    group pets(deck:pets)[Pets]

    service dog(deck:dog)[Dog] in pets
    service dragon(deck:dragon)[Dragon] in pets

    dog:R -- L:dragon
----
=====

[.column-7x.vertically-centered-panel]
=====
[graph,icons]
----
architecture-beta
...
* Link with document-level parameter `a2r-svg-icons-dir`
* Embed with the `icon` block

=== Example

[source,asciidoc,role="fit-content fragment"]
--
icon:dragon[]icon:dragon[1.4,em]icon:dragon[]
--
...
=== Parameters documentation - part I

// START PARAMETERS DOCUMENTATION 1
...
| Path to the folder containing all the external files your deck references

| `a2r-js`
|
| Path relative to the deck's assets folder
| Path to a custom JS file, loaded last in the generated deck

| `a2r-css`
|
| Path relative to the deck's assets folder
| Path to a custom CSS file, loaded last in the final deck
...
|
| Path relative to the deck's assets folder
| Location of the folder containing your SVG icons

| `a2r-page-title`
| First slide's title
| Any string
| The HTML title for the deck

| `a2r-fragment-lists`
| `false`
| booleans
| Makes all lists display line by line

| `a2r-fragment-tables`
| `false`
| booleans
| Makes all tables display row by row

|===
// END PARAMETERS DOCUMENTATION 1

=== Parameters documentation - part II

// START PARAMETERS DOCUMENTATION 2
[cols="<.^2,<.^1,<.^3,<.^2",valign="middle",role=extra-small]
|===
| Option | Default value | Accepted values | Description
...
| `a2r-theme-color`
| `[0.6,0.1,170,"oklch"]`
| Valid link:https://gka.github.io/chroma.js/#chroma[chroma-js] constructor args as JSON
...
|===
// END PARAMETERS DOCUMENTATION 2
...
* Possible to have both light & dark in the same presentation!
With manual switching or auto switching (based on browser preference)
...
Or at least add classes where they help (like adding `.no-cluster` on the first `root` of a flowchart)
